...
BEGIN
    ...
    IF v_existe_username > 0 THEN
        SELECT
            FALSE AS exito,
            'El nombre de usuario ya está en uso' AS mensaje,
            NULL AS id_usuario;
    ELSE
        ...
        IF v_existe_correo > 0 THEN
            SELECT
                FALSE AS exito,
                'El correo electrónico ya está registrado' AS mensaje,
                NULL AS id_usuario;
        ELSE
            ...
            INSERT INTO usuario (username, nombre, correo_electronico, contrasenia, rol)
            VALUES (p_username, p_nombre, p_correo, p_contrasenia, v_rol);

            -- Retornar éxito con el ID del nuevo usuario
            SELECT
                TRUE AS exito,
...
-- Valida las credenciales del usuario para el login (por username)
-- =============================================================================
...
BEGIN
    ...
    INTO v_id_usuario, v_nombre, v_rol
    FROM usuario
    WHERE username = p_username
    ...
    ELSE
        ...
        SELECT
            FALSE AS exito,
            'Usuario o contrasenia incorrectos' AS mensaje,
            NULL AS id_usuario,
            NULL AS nombre,
            NULL AS rol;
    END IF;
...
-- =============================================================================
-- PROCEDURE: sp_insertar_analisis
-- Inserta un análisis con la imagen procesada
-- Retorna el ID del análisis creado
-- Campos: id_usuario, fecha_hora (opcional), imagen_procesada
-- =============================================================================
...
BEGIN
    ...
    ELSE
        ...
        -- Insertar el análisis con todos los campos
        INSERT INTO analisis (id_usuario, fecha_hora, imagen_procesada)
        VALUES (p_id_usuario, v_fecha, p_imagen_procesada);

        SELECT
            TRUE AS exito,
            'Análisis guardado exitosamente' AS mensaje,
            LAST_INSERT_ID() AS id_analisis;
    END IF;
END;

-- =============================================================================
-- PROCEDURE: sp_insertar_objeto_detectado
-- Asocia un objeto detectado a un análisis con su confianza
-- =============================================================================
...
BEGIN
    ...
    SELECT COUNT(*) INTO v_analisis_existe
    FROM analisis
    ...
    IF v_analisis_existe = 0 THEN
        SELECT
            FALSE AS exito,
            'El análisis no existe' AS mensaje;
    ELSE
        -- Buscar o crear el objeto
        SELECT id_objeto INTO v_id_objeto
        FROM objeto
        WHERE nombre = p_nombre_objeto
        LIMIT 1;
        ...
        -- Insertar la relación objeto-análisis (ignorar si ya existe)
        INSERT IGNORE INTO Objeto_x_analisis (id_objeto, id_analisis, porcentaje_fiabilidad)
        VALUES (v_id_objeto, p_id_analisis, p_porcentaje_fiabilidad);

        SELECT
            TRUE AS exito,
            'Objeto detectado registrado' AS mensaje;
    END IF;
END;

-- =============================================================================
-- PROCEDURE: sp_guardar_resultado_completo
-- Guarda un análisis completo con todos sus objetos detectados
-- Recibe los objetos como JSON: [{"nombre": "pistola", "porcentaje_fiabilidad": 95.5}, ...]
-- =============================================================================
...
BEGIN
    ...
    ELSE
        ...
        WHILE v_i < v_total_objetos DO
            ...
            -- Insertar relación
            INSERT IGNORE INTO Objeto_x_analisis (id_objeto, id_analisis, porcentaje_fiabilidad)
            VALUES (v_id_objeto, v_id_analisis, v_porcentaje_fiabilidad);
        ...
        SELECT
            TRUE AS exito,
            'Resultado guardado exitosamente' AS mensaje,
            v_id_analisis AS id_analisis;
    END IF;
END;

-- =============================================================================
-- PROCEDURE: sp_obtener_historial_usuario
...
BEGIN
    ...
    SELECT
        ...
        a.fecha_hora,
        GROUP_CONCAT(
            CONCAT(o.nombre, ' (', ROUND(oxa.porcentaje_fiabilidad, 1), '%)')
            ORDER BY oxa.porcentaje_fiabilidad DESC
            SEPARATOR ', '
        ) AS objetos_detectados,
